...
export function ContractPanel() {
  ...
  const addrInvalid = Boolean(contractAddr.trim()) && !isValidAddress(contractAddr.trim());

  return (
    <div className="page">
      <PageHead
        icon="file-text"
        title="Smart Contracts (AML)"
        sub="Compile and deploy an AML contract, or call a method on one that is already on-chain."
      />

      <div className="card">
        {/* Real buttons, not clickable divs: these switch the whole form, and a
            keyboard user could not reach them before. */}
        <div className="tab-bar" role="tablist" aria-label="Contract action">
          <button
            type="button"
            role="tab"
          ...
          <button
            ...
            className={`tab ${tab === 'call' ? 'active' : ''}`}
            onClick={() => setTab('call')}
          >
            <Icon name="send" size={16} /> Call
          </button>
        </div>
        ...
        {tab === 'deploy' ? (
          <>
            ...
            <div className="form-actions">
              <button className="primary" onClick={requestDeploy} disabled={!source || !pin}>
                <Icon name="rocket" size={18} /> Compile &amp; Deploy
              </button>
            </div>
          </>
        ) : (
          <>
            <div className="form-row">
              ...
              <div className="input-row">
                ...
                >
                  {loadingMethods ? (
                    <>
                      <Icon name="loader" size={16} className="icon-spin" /> Fetch methods
                    </>
                  ) : (
                    <>
                      <Icon name="download" size={16} /> Fetch methods
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="form-actions">
              ...
              >
                <Icon name="signature" size={18} /> Sign &amp; Call
              </button>
            </div>
          </>
        )}
